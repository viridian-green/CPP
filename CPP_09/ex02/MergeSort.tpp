...
: m_sequence(oth.m_sequence),
m_result(oth.m_result),
...
void printResult() {
    ...
    std::printf("%.6fs\n", dur_vec);
    std::cout << "Time to process a range of " << m_sequence.size() << " elements: with std::deque : ";
    std::printf("%.6fs\n", dur_deq);
...
void vec_duration() {
    ...
    dur_vec = std::chrono::duration<double>(end - start).count();
}
...
void deq_duration() {
    ...
    FJAlgo(m_sequence);
    auto end = std::chrono::high_resolution_clock::now();
    dur_deq = std::chrono::duration<double>(end - start).count();
}
